...
import {
  Activity, Bot, Boxes, ChevronDown, ChevronRight, Code2, Database, FileBarChart, FileText,
          ...
          <section className={cn("min-h-0 min-w-0 flex-col border-r border-gray-200 bg-white xl:flex", detailOpen ? "hidden" : "flex")}>
            <div className="shrink-0 border-b border-gray-200 p-3 pb-2">
              ...
              <div className="flex gap-2">
                <label className="flex h-10 min-w-0 flex-1 items-center gap-2.5 rounded-md bg-gray-50 px-3 text-sm text-gray-600 ring-1 ring-gray-200 focus-within:ring-2 focus-within:ring-blue-600"><Search className="size-4 shrink-0" /><input className="w-full bg-transparent text-gray-950 outline-none placeholder:text-gray-400" onChange={(event) => setSearch(event.target.value)} placeholder={mode === "handler" ? "搜索算子名称、ID、领域..." : "搜索 Flow 名称、ID、步骤..."} ref={searchRef} type="search" value={search} /><kbd className="hidden rounded border border-gray-200 bg-white px-1.5 py-0.5 font-mono text-[10px] text-gray-500 sm:block">/</kbd></label>
                {developerMode ? <button aria-label={`新增${mode === "handler" ? "算子" : " Flow"}`} className="inline-flex min-h-10 shrink-0 items-center gap-1.5 rounded-md bg-blue-600 px-2.5 text-sm font-semibold text-white hover:bg-blue-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2" onClick={() => setEditor({ type: mode })} title={`新增${mode === "handler" ? "算子" : " Flow"}`} type="button"><Plus className="size-4" /><span className="hidden sm:inline">新增{mode === "handler" ? "算子" : " Flow"}</span></button> : null}
              </div>
              {mode === "handler" ? <div className="orbit-scroll mt-2 flex gap-1.5 overflow-x-auto pb-1">{categoryFilters.map(({ icon: Icon, key, label }) => <button aria-label={label} aria-pressed={activeCategory === key} className={cn("inline-flex size-10 shrink-0 items-center justify-center rounded-md bg-gray-100 text-gray-600 ring-1 ring-gray-200 transition-colors hover:bg-gray-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600", activeCategory === key && "w-auto gap-1.5 bg-blue-600 px-2.5 text-white ring-blue-600 hover:bg-blue-500")} key={key} onClick={() => setActiveCategory(key)} title={label} type="button"><Icon className="size-4" />{activeCategory === key ? <span className="whitespace-nowrap text-xs font-semibold">{label}</span> : null}</button>)}</div> : null}
            </div>
            <div className="flex min-h-10 shrink-0 items-center justify-between gap-3 border-b border-gray-200 px-3 text-xs text-gray-500"><div className="flex min-w-0 items-center gap-2"><span className="shrink-0">{visibleRecords.length} 项</span>{developerMode ? <ActivityMetricControl metric={activityMetric} onChange={setActivityMetric} /> : null}</div><span className="truncate text-right">{developerMode ? "含演示超管可见数据" : mode === "flow" ? "仅已上线 Flow" : "仅已启用算子"}</span></div>
...
function ToolbarTab({ active, icon: Icon, label, onClick }: { active: boolean; icon: LucideIcon; label: string; onClick: () => void }) { return <button aria-pressed={active} className={cn("inline-flex min-h-8 items-center gap-1.5 rounded px-2.5 text-sm font-medium text-gray-600 transition-colors hover:text-gray-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600", active && "bg-white text-gray-950 shadow-sm")} onClick={onClick} type="button"><Icon className="size-4" />{label}</button>; }
function ActivityMetricControl({ metric, onChange }: { metric: ActivityMetric; onChange: (metric: ActivityMetric) => void }) { return <div aria-label="研发视角统计口径" className="flex rounded bg-gray-100 p-0.5 ring-1 ring-gray-200">{(["total", "7d", "30d"] as ActivityMetric[]).map((option) => <button aria-pressed={metric === option} className={cn("min-h-7 rounded px-2 text-[11px] font-semibold text-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600", metric === option && "bg-white text-blue-700 shadow-sm")} key={option} onClick={() => onChange(option)} type="button">{getActivityMetricLabel(option)}</button>)}</div>; }
