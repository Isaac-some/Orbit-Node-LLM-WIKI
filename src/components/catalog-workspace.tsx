...
import {
  Bot, Boxes, ChevronDown, ChevronRight, Code2, Database, FileBarChart, FileText,
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(360px,44%)_minmax(0,1fr)]">
          <section className={cn("min-h-0 min-w-0 flex-col border-r border-gray-200 bg-white xl:flex", detailOpen ? "hidden" : "flex")}>
            ...
            <div className="orbit-scroll min-h-0 flex-1 overflow-y-auto p-2">{visibleRecords.length ? <ul>{visibleRecords.map((record) => <CatalogRow active={recordId(record) === selectedIds[mode]} key={`${record.entityType}-${recordId(record)}`} onSelect={() => selectRecord(record)} record={record} />)}</ul> : <EmptyState mode={mode} searching={Boolean(search.trim())} />}</div>
          </section>
...
function searchableText(record: CatalogRecord) { return record.entityType === "handler" ? [record.handlerId, record.displayName, record.intro, ...record.domains.map((domain) => domainLabels[domain])].join(" ") : [record.flowId, record.displayName, record.intro, record.serviceDomain, ...record.steps.map((step) => step.displayName)].join(" "); }
